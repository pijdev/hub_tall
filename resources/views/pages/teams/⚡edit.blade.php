...
new class extends Component
{
    ...
    public function updateTeam(): void
    {
        ...
        Flux::toast(variant: 'success', text: __('Time atualizado.'));
    ...
    public function render()
    {
        ...
        $title = $this->permissions->canUpdateTeam
            ? __('Editar :name', ['name' => $teamName])
            : __('Visualizar :name', ['name' => $teamName]);
...
<section class="w-full">
    ...
    <x-pages::settings.layout :heading="__('Times')" :subheading="__('Gerencie as configurações do time')">
        <div class="space-y-10">
            <div class="space-y-6">
                @if ($this->permissions->canUpdateTeam)
                    <div class="space-y-4">
                        <form wire:submit="updateTeam" class="space-y-6">
                            <flux:input wire:model="teamName" :label="__('Nome do time')" required data-test="team-name-input" />
            ...
            <div class="space-y-6">
                <div class="flex items-center justify-between">
                    <div>
                        <flux:heading>{{ __('Membros do time') }}</flux:heading>
                        @if ($this->permissions->canAddMember || $this->permissions->canUpdateMember || $this->permissions->canRemoveMember)
                            <flux:subheading>{{ __('Gerencie quem pertence a este time') }}</flux:subheading>
                        @endif
                    </div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @if ($member['role'] !== 'owner' && $this->permissions->canRemoveMember)
                                    <flux:modal.trigger name="remove-member-{{ $member['id'] }}">
                                        <flux:tooltip :content="__('Remover membro')">
                                            <flux:button
                                                variant="ghost"
                                                size="sm"
                                                icon="x-mark"
            ...
            @if (count($invitations) > 0)
                <div class="space-y-6">
                    <div>
                        <flux:heading>{{ __('Convites pendentes') }}</flux:heading>
                        <flux:subheading>{{ __('Convites que ainda não foram aceitos') }}</flux:subheading>
                    </div>
                            ...
                            <div class="flex items-center justify-between rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900" data-test="invitation-row">
                                ...
                                @if ($this->permissions->canCancelInvitation)
                                    <flux:modal.trigger name="cancel-invitation-{{ $invitation['code'] }}">
                                        <flux:tooltip :content="__('Cancelar convite')">
                                            <flux:button
                                                variant="ghost"
                                                size="sm"
                                                icon="x-mark"
            ...
            @if ($this->permissions->canDeleteTeam && ! $teamData['is_personal'])
                <div class="space-y-6">
                    <div>
                        <flux:heading>{{ __('Excluir time') }}</flux:heading>
                        <flux:subheading>{{ __('Excluir permanentemente seu time') }}</flux:subheading>
                    </div>
                    ...
                    <div class="space-y-4 rounded-lg border border-red-200 bg-red-50 p-4 text-red-700 dark:border-red-200/10 dark:bg-red-900/20 dark:text-red-100">
                        <div>
                            <p class="font-medium">{{ __('Atenção') }}</p>
                            <p class="text-sm">{{ __('Tenha cuidado, esta ação não pode ser desfeita.') }}</p>
                        </div>
