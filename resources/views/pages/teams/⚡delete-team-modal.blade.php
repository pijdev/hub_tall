...
new class extends Component {
    ...
    {
        return __('Digite ":name" para confirmar', ['name' => $this->team->name]);
    }
    ...
    public function deleteTeam(): void
    {
        ...
        if ($validated['deleteName'] !== $this->team->name) {
            $this->addError('deleteName', __('O nome do time não confere.'));
        ...
        Flux::toast(variant: 'success', text: __('Time excluído.'));
...
<flux:modal name="delete-team" :show="$errors->isNotEmpty()" focusable class="max-w-lg">
    <form wire:submit="deleteTeam" class="space-y-6">
        <div>
            <flux:heading size="lg">{{ __('Tem certeza?') }}</flux:heading>
            <flux:subheading>
                {{ __('Esta ação não pode ser desfeita. Isso excluirá permanentemente o time ":name".', ['name' => $team->name]) }}
            </flux:subheading>
        </div>
