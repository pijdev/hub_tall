...
new class extends Component {
    ...
    public function createInvitation(): void
    {
        ...
        Flux::toast(variant: 'success', text: __('Convite enviado.'));
...
<flux:modal name="invite-member" :show="$errors->isNotEmpty()" focusable class="max-w-lg">
    <form wire:submit="createInvitation" class="space-y-6">
        <div>
            <flux:heading size="lg">{{ __('Convidar um membro para o time') }}</flux:heading>
            <flux:subheading>{{ __('Enviar um convite para entrar neste time.') }}</flux:subheading>
        </div>

        <div class="space-y-4">
            <flux:input wire:model="inviteEmail" type="email" :label="__('E-mail')" required data-test="invite-email" />

            <flux:select wire:model="inviteRole" :label="__('Função')" data-test="invite-role">
        ...
        <div class="flex justify-end space-x-2 rtl:space-x-reverse">
            ...
            <flux:button variant="primary" type="submit" data-test="invite-submit">{{ __('Enviar convite') }}</flux:button>
        </div>
    </form>
</flux:modal>
