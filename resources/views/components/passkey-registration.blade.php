...
>
    <template x-if="!supported">
        <flux:text>{{ __('Chaves de acesso não são suportadas neste navegador.') }}</flux:text>
    </template>
    ...
    <template x-if="supported && showForm">
        <div class="space-y-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 p-4">
            ...
            />
            <flux:text class="!mt-1">{{ __('Dê um nome a esta chave para identificá-la depois.') }}</flux:text>
            ...
            <div class="flex gap-2">
                ...
                >
                    <span x-show="!loading">{{ __('Registrar chave de acesso') }}</span>
